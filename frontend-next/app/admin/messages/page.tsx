...
type Msg = {
  ...
  body: string;
  createdAt: string;
  sender: UserBrief & { role: string };
};

export default function AdminMessagesPage() {
  const [list, setList] = useState<ConvSummary[] | null>(null);
  const [selectedId, setSelectedId] = useState<string | null>(null);
  const [threadUser, setThreadUser] = useState<UserBrief | null>(null);
  ...
  const loadList = useCallback(async () => {
    ...
    setListError(null);
    try {
      const res = await fetch(apiUrl("admin/support/conversations"), {
        cache: "no-store",
        headers: authHeaders(),
      });
  ...
  const loadThread = useCallback(async (id: string) => {
    ...
    try {
      const res = await fetch(
        apiUrl(`admin/support/conversations/${id}/messages`),
      ...
      setThreadUser(data.conversation.user);
  ...
  const onSend = async () => {
    ...
    try {
      const res = await fetch(
        apiUrl(`admin/support/conversations/${selectedId}/messages`),
        {
          method: "POST",
          headers: {
            ...authHeaders(),
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">
          Open a conversation to read the full thread and reply to clients.
        </p>
      </div>

      <div className="flex min-h-[420px] flex-col gap-4 rounded-xl border border-slate-200 bg-white shadow-sm lg:flex-row lg:gap-0">
        <div className="flex shrink-0 flex-col border-slate-200 lg:w-80 lg:border-r">
        ...
        <div className="flex min-h-[320px] min-w-0 flex-1 flex-col">
          ...
          {selectedId && threadUser && subject && (
            <>
              <div className="border-b border-slate-200 px-4 py-3">
                <h2 className="font-semibold text-slate-900">{subject}</h2>
                <p className="text-xs text-slate-600">
                  {threadUser.firstName} {threadUser.lastName} · {threadUser.email}
                </p>
              </div>
              {threadError && (
                      ...
                      >
                        ...
                        <p
                          className={`mt-1 text-[10px] ${
                            fromAdmin ? "text-slate-300" : "text-slate-500"
                          }`}
                        >
              ...
              <div className="border-t border-slate-200 p-4">
                ...
                <textarea
                  ...
                  className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 outline-none ring-slate-900/5 focus:border-slate-400 focus:ring-2 focus:ring-slate-900/10"
                />
